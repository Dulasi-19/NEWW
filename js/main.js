$(function () {
    ...
    $('.open-contact').on('click', function (e) {
        ...
        $('#popupContent').removeClass('scale-90').addClass('scale-100');
    });

    // handle popup form submit (simple mimic)
    $('#contactFormPopup').on('submit', function (e) {
        e.preventDefault();
        var alertBox = $('<div style="position:fixed; top:20px; right:20px; background:#ff8c05; color:#fff; padding:15px 30px; border-radius:5px; z-index:99999;">Message Sent Successfully!</div>');
        ...
        setTimeout(function () {
            ...
            if (typeof closeContactPopup === 'function') {
                ...
                $('#popupContent').removeClass('scale-100').addClass('scale-90');
            }
        }, 2000);
    });
